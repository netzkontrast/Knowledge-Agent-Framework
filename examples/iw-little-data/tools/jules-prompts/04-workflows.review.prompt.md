# Jules Panel-Review Session: `04-workflows`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/04-workflows.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/04-workflows.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-WF-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/04-workflows.review.md` in folgendem Format:
...
### S-WF-001 <original scenario title>
...
(repeat for every S-WF-NNN scenario in the file)
...
3. `git add little-data/data/reviews/04-workflows.review.md`
4. `git commit -m "review: panel-review für 04-workflows (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 04-workflows: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/04-workflows.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
