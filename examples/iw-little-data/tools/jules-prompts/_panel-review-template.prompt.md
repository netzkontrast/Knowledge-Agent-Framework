# Jules Panel-Review Session: `${FILE_NAME}`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/${FILE_NAME}.md` (auf Branch `${BRANCH}`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/${FILE_NAME}.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-${FILE_PREFIX}-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/${FILE_NAME}.review.md` in folgendem Format:
...
### S-${FILE_PREFIX}-001 <original scenario title>
...
(repeat for every S-${FILE_PREFIX}-NNN scenario in the file)
...
1. `git pull origin ${BRANCH}` zum Start.
2. Datei lesen, Plan abstimmen, Panel-Reviews schreiben.
3. `git add little-data/data/reviews/${FILE_NAME}.review.md`
4. `git commit -m "review: panel-review für ${FILE_NAME} (Jules <session-id>)"`
5. `git push origin ${BRANCH}`

## Status-Zeile am Ende

> "Done. Panel-Review für ${FILE_NAME}: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/${FILE_NAME}.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
