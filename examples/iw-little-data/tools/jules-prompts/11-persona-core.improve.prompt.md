# Jules Cross-Ref + Advanced Improvement Session: `11-persona-core`
...
1. **Knowledge-Datei (aktueller Stand)** — `little-data/langdock-deploy/knowledge/11-persona-core.md` (auf Branch `claude/friendly-ride-eRsPr`). Das ist deine Basis. **Erhalte vorhandene Trigger und Strategische Ziele.**
2. **Panel-Review** — `little-data/data/reviews/11-persona-core.review.md`. Diese Datei enthält pro Szenario das Panel-Urteil (KEEP / IMPROVE / DROP), Improvement-Notes, und Cross-Ref-Vorschläge. **Du MUSST jede IMPROVE-Note umsetzen.**
3. **50 Advanced Scenarios (Julia-Lens)** — `little-data/data/research/50-advanced-scenarios-julia-lens.md`. Identifiziere die Rows in der "Integration-Plan"-Sektion, die `11-persona-core` als Target-File nennen. **Diese Rows musst du als neue Szenarien anfügen (Numerisierung weiterführen ab S-LC-105).**
...
3. Behalte die **Szenario-ID** unverändert (S-LC-001 bleibt S-LC-001).
4. Behalte die **Reihenfolge** unverändert.

Für jedes Szenario, das **DROP** markiert ist:

1. Entferne das Szenario.
2. Renumeriere folgende Szenarien NICHT — lasse die Lücke (S-LC-001, S-LC-003, …). Notiere am Datei-Ende eine `## Dropped Scenarios` Sektion mit den entfernten IDs + Begründung.
...
2. Beispiel: `Anschluss-Szenario: S-LC-005 oder siehe auch 03-wissensordner-und-rag § "Wissensordner-Setup für CRM-Daten"`.
...
1. Identifiziere die Rows, die `11-persona-core` als Target-File nennen.
2. Für jede solche Row, schreibe ein vollständiges H3-Szenario mit der nächsten freien ID (z.B. S-LC-105 für die erste Advanced).
...
Schreibe nach `little-data/langdock-deploy/knowledge/11-persona-core.md` (überschreibt die existierende Version mit der verbesserten).
...
### S-LC-105 <Advanced-Scenario aus Julia-Lens A-NN>
(neu — Advanced)
...
|---|---|---|
| S-LC-005 | 03-wissensordner-und-rag § "Wissensordner-Setup" | gemeinsame CRM-Daten-Ingest-Logic |
| S-LC-014 | 07-modelle-und-kosten § "Heavy-Hitter Detection" | Token-Budget-Trigger |
| … | … | … |
```

## Validierung vor Commit

1. `bash little-data/tools/check_schema.sh little-data/langdock-deploy/knowledge/11-persona-core.md` — muss PASS sein.
...
```bash
git add little-data/langdock-deploy/knowledge/11-persona-core.md
git commit -m "improve: cross-refs + advanced scenarios in 11-persona-core (Jules <session-id>)"
git push origin claude/friendly-ride-eRsPr
```

## Status-Zeile am Ende

> "Done. 11-persona-core improved: [original count] Szenarien → [final count] (KEEP NN, IMPROVE NN, DROP NN, ADVANCED NN). check_schema.sh PASS. Cross-Refs: NN. Commit [SHA] pushed."
...
- NIEMALS Advanced Scenarios, deren `Target file` nicht 11-persona-core ist, in diese Datei aufnehmen.
