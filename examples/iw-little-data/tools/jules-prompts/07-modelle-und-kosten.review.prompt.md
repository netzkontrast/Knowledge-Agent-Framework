# Jules Panel-Review Session: `07-modelle-und-kosten`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/07-modelle-und-kosten.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/07-modelle-und-kosten.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-MK-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/07-modelle-und-kosten.review.md` in folgendem Format:

```markdown
# Review: 07-modelle-und-kosten
...
### S-MK-001 <original scenario title>
...
(repeat for every S-MK-NNN scenario in the file)
...
3. `git add little-data/data/reviews/07-modelle-und-kosten.review.md`
4. `git commit -m "review: panel-review für 07-modelle-und-kosten (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 07-modelle-und-kosten: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/07-modelle-und-kosten.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
