# Jules Panel-Review Session: `13-data-agent-anweisungen-pro-thema`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/13-data-agent-anweisungen-pro-thema.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/13-data-agent-anweisungen-pro-thema.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-DA-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/13-data-agent-anweisungen-pro-thema.review.md` in folgendem Format:

```markdown
# Review: 13-data-agent-anweisungen-pro-thema
...
### S-DA-001 <original scenario title>
...
(repeat for every S-DA-NNN scenario in the file)
...
3. `git add little-data/data/reviews/13-data-agent-anweisungen-pro-thema.review.md`
4. `git commit -m "review: panel-review für 13-data-agent-anweisungen-pro-thema (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 13-data-agent-anweisungen-pro-thema: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/13-data-agent-anweisungen-pro-thema.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
