# Jules Cross-Ref + Advanced Improvement Session: `09-marketing-praxis`
...
1. **Knowledge-Datei (aktueller Stand)** — `little-data/langdock-deploy/knowledge/09-marketing-praxis.md` (auf Branch `claude/friendly-ride-eRsPr`). Das ist deine Basis. **Erhalte vorhandene Trigger und Strategische Ziele.**
2. **Panel-Review** — `little-data/data/reviews/09-marketing-praxis.review.md`. Diese Datei enthält pro Szenario das Panel-Urteil (KEEP / IMPROVE / DROP), Improvement-Notes, und Cross-Ref-Vorschläge. **Du MUSST jede IMPROVE-Note umsetzen.**
3. **50 Advanced Scenarios (Julia-Lens)** — `little-data/data/research/50-advanced-scenarios-julia-lens.md`. Identifiziere die Rows in der "Integration-Plan"-Sektion, die `09-marketing-praxis` als Target-File nennen. **Diese Rows musst du als neue Szenarien anfügen (Numerisierung weiterführen ab S-MP-105).**
...
3. Behalte die **Szenario-ID** unverändert (S-MP-001 bleibt S-MP-001).
4. Behalte die **Reihenfolge** unverändert.

Für jedes Szenario, das **DROP** markiert ist:

1. Entferne das Szenario.
2. Renumeriere folgende Szenarien NICHT — lasse die Lücke (S-MP-001, S-MP-003, …). Notiere am Datei-Ende eine `## Dropped Scenarios` Sektion mit den entfernten IDs + Begründung.
...
2. Beispiel: `Anschluss-Szenario: S-MP-005 oder siehe auch 03-wissensordner-und-rag § "Wissensordner-Setup für CRM-Daten"`.
...
1. Identifiziere die Rows, die `09-marketing-praxis` als Target-File nennen.
2. Für jede solche Row, schreibe ein vollständiges H3-Szenario mit der nächsten freien ID (z.B. S-MP-105 für die erste Advanced).
...
Schreibe nach `little-data/langdock-deploy/knowledge/09-marketing-praxis.md` (überschreibt die existierende Version mit der verbesserten).
...
### S-MP-105 <Advanced-Scenario aus Julia-Lens A-NN>
(neu — Advanced)
...
|---|---|---|
| S-MP-005 | 03-wissensordner-und-rag § "Wissensordner-Setup" | gemeinsame CRM-Daten-Ingest-Logic |
| S-MP-014 | 07-modelle-und-kosten § "Heavy-Hitter Detection" | Token-Budget-Trigger |
| … | … | … |
```

## Validierung vor Commit

1. `bash little-data/tools/check_schema.sh little-data/langdock-deploy/knowledge/09-marketing-praxis.md` — muss PASS sein.
...
```bash
git add little-data/langdock-deploy/knowledge/09-marketing-praxis.md
git commit -m "improve: cross-refs + advanced scenarios in 09-marketing-praxis (Jules <session-id>)"
git push origin claude/friendly-ride-eRsPr
```

## Status-Zeile am Ende

> "Done. 09-marketing-praxis improved: [original count] Szenarien → [final count] (KEEP NN, IMPROVE NN, DROP NN, ADVANCED NN). check_schema.sh PASS. Cross-Refs: NN. Commit [SHA] pushed."
...
- NIEMALS Advanced Scenarios, deren `Target file` nicht 09-marketing-praxis ist, in diese Datei aufnehmen.
