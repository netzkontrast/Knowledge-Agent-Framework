...
- **2026-06-02 (loop 4, files 04+06 best-typed):** file 04 (Workflows) → 79 `Workflow:`+`Budget:` + 1 Decision; file 06 (API/Deployment) → 72 Guide-`Vorlage:` + 7 Decision + 1 Workflow. **R20 learning — Vorgehen↔Vorlage duplication:** when a P→G conversion adds a `Vorlage:` skeleton, the pre-existing verbose `Vorgehen:` steps duplicate it and inflate the chunk past the R13 warn band. **Discipline: condense the Vorgehen to terse process-steps IN THE SAME PASS as the G conversion** (the substantive detail moves into the Vorlage; grounded facts must be preserved there). Applied retroactively to file 06 (median 2820→2728 B). **R20 learning — G is the honest type for advisory files:** "api/deployment" filenames mislead; when Little Data *berät* (delivers a briefing/dossier/spec/blueprint), the type is **G**, not **A/M/W**, even for Search/Streaming/Webhook/Vision scenarios — reserve A/M/W for scenarios whose deliverable is the executable call/pipeline itself.
- **2026-06-02 (loop 3 close):** added **R18** Solution-Chunk-Schemata catalog (nine chunk-type codes P/A/M/S/T/W/C/D/G with type-specific payloads, scenario-ID rename convention `S-XXX-Y-NNN`) and **R19** terse field-marker schema (~167 B / scenario saved, ~185 KB total, embedding signal substance-dominated). New file: `19-iwmedien-zusammenarbeit.md` (kind iw-brand, R7b, 10 scenarios). R7a Konkrete Empfehlung rolled out to files 11/12/14/19 (120 hand-crafted fields, zero template feel per spec-panel review). Spec-panel loop-3 review verdict: **ITERATE LOOP 4 — tag `v0.9-RC` now**. Catalog confirmed (no 10th type needed); biggest discovery: **G (Guide/Document) far more common than expected** — files 05/06 are G-dominant, not A/M-dominant as filenames suggest. Loop-3 review ledger: `docs/superpowers/specs/v2-review-loop3.md`.
