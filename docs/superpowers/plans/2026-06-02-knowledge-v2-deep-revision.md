...
- [~] **R20 best-type re-authoring** (per file): P → Workflow/MCP/Skill/Code/Config/Decision/Guide where genuinely better, source-grounded. Order: 04 (pure W) → 06 (G/D/A) → 05 (G-dominant) → 10 (S/P) → 00-03,07-09 → 14/16/17.
  ...
  - [x] **File 17 (branchen-think-tank-praxis) DONE** — added R7a Empfehlung to all 39 (0->39). Types: 26 P (content/analysis output) / 11 G (embargo choreography, 8-week editorial calendar, SEO topic-hub architecture, transparency+spin-check, fact-verification protocol, governance checklist, internal Q&A guideline, forecast-comm template, embargo-breach playbook, license checklist, topic roadmap) / 1 W (S-TT-013 newsletter-/podcast-repurposing automation) / 1 D (S-TT-029 visualization-option recommendation). median 2131 B; all gates PASS.
  - [x] **All scenario knowledge files now R20-complete** (00-10, 14, 16, 17; 11-13 persona/anweisung and 15 glossar follow their own kinds). Marker-spacing formatter applied repo-wide.
