...
- [~] **R20 best-type re-authoring** (per file): P → Workflow/MCP/Skill/Code/Config/Decision/Guide where genuinely better, source-grounded. Order: 04 (pure W) → 06 (G/D/A) → 05 (G-dominant) → 10 (S/P) → 00-03,07-09 → 14/16/17.
  ...
  - [ ] **File 10 next** (Prompts/Skills) — expect `Skill:`+`Trigger-Wörter:` (S-type) for reusable micro-tasks + `Prompt:` (genuine one-off generations stay P) + some G. This is the file where P legitimately survives most.
  - [ ] **Then:** 00–03, 07–09, 14/16/17.
