...
> **Part of:** the **Knowledge Agent Framework** — the bite-sized execution plan that turns the build-design into concrete steps.
> **Companion spec:** `docs/knowledge-build-plan-design.md`
> **Status:** execution plan; updates as phases complete.
> **Reference example:** the IW Little Data build (see `examples/iw-little-data/`). The concrete paths (`examples/iw-little-data/…`), file numbers, and statuses below are that build's instantiation of this plan; reuse the phase/gate structure for any new agent.

This plan turns the build-design into concrete steps. Phase 0/1/1.5/3/4/5 are bite-sized (one tool invocation per step). Phase 2 is phase-oriented (one authoring session per knowledge file — a local subagent or in-session author).
...
|---|---|---|---|
| 0.1 | Create directory skeleton (`examples/iw-little-data/{tools,data/extracts,langdock-deploy/knowledge}`, `docs/`) | Bash | ✅ done |
| 0.2 | Write `examples/iw-little-data/tools/check_prompt_size.sh` | Write | ✅ done |
| 0.3 | Write `examples/iw-little-data/tools/check_prompt_tokens.py` (tiktoken-based) | Write | ✅ done |
| 0.4 | Write `examples/iw-little-data/tools/check_coverage.sh` | Write | ✅ done |
| 0.5 | Write `examples/iw-little-data/tools/check_overlap.py` (sentence-transformers stub) | Write | ✅ done |
| 0.6 | Write `examples/iw-little-data/data/coverage-matrix.md` skeleton | Write | ✅ done |
| 0.7 | Write this plan file | Write | ✅ done |
| 0.8 | Draft Knowledge-File-Authoring-Spec `docs/knowledge-file-authoring-spec.md` | Write (post-T2/T7/T8) | pending |
| 0.9 | Draft System-Prompt v3 `examples/iw-little-data/langdock-deploy/AGENT_PROMPT.md` | Write (post-T5/T7) | pending |
...
Each step dispatches one `sc:sc-deep-research` subagent (model = haiku) that writes its output directly to `examples/iw-little-data/data/extracts/<T-X>.md`. Subagents run in parallel; no shared state.
...
| 1.T7 | Persona-via-Knowledge Framework | research/07 + persona/style/build references | `extracts/T7-persona-framework.md` | ✅ done (293 L / 17 KB) |
...
## Phase 0.5 — Persona-Doc Preparation (vorab, vor Phase 2)

**Per user directive 2026-05-31:** the persona files (files 11 + 12) are prepared BEFORE Phase 2 starts. They become the persona reference handed to every Phase-2 authoring session. Without them, the authoring sessions for files 00-09, 10, 13 would lack consistent persona grounding.

| Step | Action | Owner | Status |
|---|---|---|---|
| 0.5.1 | Wait for T5 (Data Canon) extract to complete | (background) | ✅ done |
| 0.5.2 | Dispatched authoring session + recovered patch locally for `11-persona-core.md` — inputs: T4, T5, T7, T8 + research/05,06,07,08,11 sources + Authoring-Spec | Agent | pending |
...
| 3.3 | Write `examples/iw-little-data/langdock-deploy/CONVERSATION_STARTERS.md` (10 in DE, ≤255 chars each) | Write | pending |
| 3.4 | Write `examples/iw-little-data/examples/good-outputs.md` (10-14 worked examples in F9 gestaffelt form) | Write | pending |
| 3.5 | Write `examples/iw-little-data/examples/bad-outputs.md` (anti-patterns) | Write | pending |
| 3.6 | Finalize `examples/iw-little-data/langdock-deploy/AGENT_PROMPT.md` v3 (apply T7 refinements: voice anchors, modes, vocabulary) | Edit | pending |
| 3.7 | Run `check_prompt_size.sh` + `check_prompt_tokens.py` — must pass | Bash | pending |
| 3.8 | Write `examples/iw-little-data/MAINTENANCE.md` (review cadence, staleness rules) | Write | pending |
...
| Step | Action | Owner | Status |
|---|---|---|---|
| 5.1 | Write `examples/iw-little-data/INSTALL.md` — step-by-step Langdock-Workspace setup, Wissensordner-Upload, Agent-Konfiguration, Konversations-Starter-Setup, validation rollout | Write | pending |
| 5.2 | Write `examples/iw-little-data/README.md` — project overview for PR reviewers + future maintainers | Write | pending |
...
- **R1 — Authoring dispatcher unavailable in Phase 2.** Fall back to local `general-purpose` subagents (one per file). Same prompts, different dispatcher.
