# Jules Work-Unit: little-data Knowledge-File `02-agenten-konfiguration`
...
Schreibe die Knowledge-Datei `little-data/langdock-deploy/knowledge/02-agenten-konfiguration.md` für den Langdock-Wissensordner des "Little Data" Beratungsagenten. Zielgruppe: eine strategisch arbeitende, hands-on-orientierte Marketing-Direktorin (DACH).
...
3. `little-data/data/coverage-matrix.md` — die Rows für deine Datei `02-agenten-konfiguration`.
...
| Feld | Wert |
|---|---|
| **Dateipfad** | `little-data/langdock-deploy/knowledge/02-agenten-konfiguration.md` |
| **H1** | `Agenten-Konfiguration für Marketing-Direktoren` |
| **Pflicht-H2-Sub-Topics** (in dieser Reihenfolge) | - Agent-Erstellung Schritt für Schritt
...
| **Marketing-Funktionen-Fokus** | MarketingOps, Brand, Content, Analytics |
...
### S-AK-001 [Szenario-Titel auf Deutsch]
...
Schreibe die Datei via `Write` direkt nach `little-data/langdock-deploy/knowledge/02-agenten-konfiguration.md`. Erstelle danach einen sauberen Commit auf deinem Working-Branch mit dem Format `little-data: author 02-agenten-konfiguration via Jules`.
...
> "Done. 02-agenten-konfiguration: [N_lines] lines, [N_bytes] bytes, [N_scenarios] scenarios. Method-distribution: [Mxx: count, ...]. Validation: PASS [optional notes]."
