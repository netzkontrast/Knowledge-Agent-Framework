# Jules Work-Unit: little-data Knowledge-File `07-modelle-und-kosten`
...
Schreibe die Knowledge-Datei `little-data/langdock-deploy/knowledge/07-modelle-und-kosten.md` für den Langdock-Wissensordner des "Little Data" Beratungsagenten. Zielgruppe: eine strategisch arbeitende, hands-on-orientierte Marketing-Direktorin (DACH).
...
3. `little-data/data/coverage-matrix.md` — die Rows für deine Datei `07-modelle-und-kosten`.
...
- little-data/data/extracts/T6-dach-kontext-kosten.md


### Research-Source-Files (Ground Truth — bei Konflikt gewinnen diese über Extracts)

- little-data/data/research/01-langdock-platform-feature-inventory.md
- little-data/data/research/02-dach-marketing-ai-adoption.md
- little-data/data/sources/02-langdock-developer-overview-and-integrations.md
- little-data/data/sources/03-langdock-developer-best-practices-guide.md
- little-data/data/sources/06-langdock-playbook.md
...
| Feld | Wert |
|---|---|
| **Dateipfad** | `little-data/langdock-deploy/knowledge/07-modelle-und-kosten.md` |
| **H1** | `Modelle und Kosten für Marketing-Direktoren` |
| **Pflicht-H2-Sub-Topics** (in dieser Reihenfolge) | - Modell-Katalog für Marketing-Aufgaben (Tabelle mit EU-Preisen)
- Modell-Empfehlungen für Content-Drafting (Gemini 2.5 Flash / Sonnet 4.6 / Haiku 4.5)
- Modell-Empfehlungen für Briefing-Erstellung
- Modell-Empfehlungen für strategische Analyse (Opus 4.7 / GPT-5 Pro)
- Provider-Vergleich nach Anwendungsfall (Google vs Anthropic vs OpenAI vs Mistral vs Llama)
- Auto Mode (Vorsicht für Beginner — kann teure Modelle feuern)
- BYOK (Bring Your Own Key) — wann es sich lohnt
- Workspace- und Workflow-Limits (€500/Monat default)
- Warn-Schwellen 50%/75%/90%
- Fair Usage Policy (5h Session, Mo-Mo Weekly, 250 Messages/3h)
- Pricing-Tiers (Trial / Standard €25 / Max €99 / Enterprise)
- Cost-Saving-Patterns für Marketing-Teams
 |
| **Anchor-Strings** (verbatim in den ersten Chunks der jeweiligen Sektionen) | (keine) |
| **Marketing-Funktionen-Fokus** | alle Funktionen (alle haben Modell-Wahl) |
...
### S-MK-001 [Szenario-Titel auf Deutsch]
...
Schreibe die Datei via `Write` direkt nach `little-data/langdock-deploy/knowledge/07-modelle-und-kosten.md`. Erstelle danach einen sauberen Commit auf deinem Working-Branch mit dem Format `little-data: author 07-modelle-und-kosten via Jules`.
...
> "Done. 07-modelle-und-kosten: [N_lines] lines, [N_bytes] bytes, [N_scenarios] scenarios. Method-distribution: [Mxx: count, ...]. Validation: PASS [optional notes]."
