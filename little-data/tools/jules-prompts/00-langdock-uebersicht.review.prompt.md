# Jules Panel-Review Session: `00-langdock-uebersicht`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/00-langdock-uebersicht.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/00-langdock-uebersicht.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-LU-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/00-langdock-uebersicht.review.md` in folgendem Format:

```markdown
# Review: 00-langdock-uebersicht
...
### S-LU-001 <original scenario title>
...
(repeat for every S-LU-NNN scenario in the file)
...
3. `git add little-data/data/reviews/00-langdock-uebersicht.review.md`
4. `git commit -m "review: panel-review für 00-langdock-uebersicht (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 00-langdock-uebersicht: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/00-langdock-uebersicht.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
