# Jules Work-Unit: little-data Knowledge-File `04-workflows`
...
Schreibe die Knowledge-Datei `little-data/langdock-deploy/knowledge/04-workflows.md` für den Langdock-Wissensordner des "Little Data" Beratungsagenten. Zielgruppe: eine strategisch arbeitende, hands-on-orientierte Marketing-Direktorin (DACH).
...
3. `little-data/data/coverage-matrix.md` — die Rows für deine Datei `04-workflows`.
...
- little-data/data/sources/04-langdock-agent-spec-development.md
...
| Feld | Wert |
|---|---|
| **Dateipfad** | `little-data/langdock-deploy/knowledge/04-workflows.md` |
| **H1** | `Workflows für Marketing-Direktoren (Beratung)` |
| **Pflicht-H2-Sub-Topics** (in dieser Reihenfolge) | - Was Workflows sind und wann sie sich lohnen
...
| **Marketing-Funktionen-Fokus** | MarketingOps, Content, CRM, Performance, Analytics |
...
### S-WF-001 [Szenario-Titel auf Deutsch]
...
Schreibe die Datei via `Write` direkt nach `little-data/langdock-deploy/knowledge/04-workflows.md`. Erstelle danach einen sauberen Commit auf deinem Working-Branch mit dem Format `little-data: author 04-workflows via Jules`.
...
> "Done. 04-workflows: [N_lines] lines, [N_bytes] bytes, [N_scenarios] scenarios. Method-distribution: [Mxx: count, ...]. Validation: PASS [optional notes]."
