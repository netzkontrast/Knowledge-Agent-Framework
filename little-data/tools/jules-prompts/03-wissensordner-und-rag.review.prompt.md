# Jules Panel-Review Session: `03-wissensordner-und-rag`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/03-wissensordner-und-rag.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/03-wissensordner-und-rag.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-WR-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/03-wissensordner-und-rag.review.md` in folgendem Format:

```markdown
# Review: 03-wissensordner-und-rag
...
### S-WR-001 <original scenario title>
...
(repeat for every S-WR-NNN scenario in the file)
...
3. `git add little-data/data/reviews/03-wissensordner-und-rag.review.md`
4. `git commit -m "review: panel-review für 03-wissensordner-und-rag (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 03-wissensordner-und-rag: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/03-wissensordner-und-rag.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
