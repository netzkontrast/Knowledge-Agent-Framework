# Jules Panel-Review Session: `02-agenten-konfiguration`
...
1. **Knowledge-Datei unter Review** — `little-data/langdock-deploy/knowledge/02-agenten-konfiguration.md` (auf Branch `claude/friendly-ride-eRsPr`). Lies das gesamte Dokument vor Beginn.
2. **Original-Dispatch-Prompt** (der Auftrag, mit dem diese Datei geschrieben wurde) — `little-data/tools/jules-prompts/02-agenten-konfiguration.prompt.md`. Dieser Prompt definiert die Erwartungen.
...
Für **jedes** `### S-AK-NNN` Szenario in der Datei führe einen simulierten **Business-Expert-Panel** durch. Das Panel besteht aus neun Experten (Quelle: `sc:sc-business-panel-experts`):
...
Schreibe nach `little-data/data/reviews/02-agenten-konfiguration.review.md` in folgendem Format:

```markdown
# Review: 02-agenten-konfiguration
...
### S-AK-001 <original scenario title>
...
(repeat for every S-AK-NNN scenario in the file)
...
3. `git add little-data/data/reviews/02-agenten-konfiguration.review.md`
4. `git commit -m "review: panel-review für 02-agenten-konfiguration (Jules <session-id>)"`
5. `git push origin claude/friendly-ride-eRsPr`

## Status-Zeile am Ende

> "Done. Panel-Review für 02-agenten-konfiguration: [count] Szenarien reviewed. Verdicts: KEEP NN / IMPROVE NN / DROP NN. Datei: little-data/data/reviews/02-agenten-konfiguration.review.md ([bytes] bytes). Cross-Ref-Vorschläge: [count] zu [list of files]."
