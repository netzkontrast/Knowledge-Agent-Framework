# Jules Work-Unit: little-data Knowledge-File `${FILE_NAME}`

Du bist eine Jules-Session mit Schreibzugriff auf das Repo `netzkontrast/soul.md` auf Branch `${BRANCH}`. Diese Aufgabe ist self-contained — folge ihr genau.

## Aufgabe

Schreibe die Knowledge-Datei `little-data/langdock-deploy/knowledge/${FILE_NAME}.md` für den Langdock-Wissensordner des "Little Data" Beratungsagenten. Zielgruppe: eine strategisch arbeitende, hands-on-orientierte Marketing-Direktorin (DACH).
...
3. `little-data/data/coverage-matrix.md` — die Rows für deine Datei `${FILE_NAME}`.
...
| Feld | Wert |
|---|---|
| **Dateipfad** | `little-data/langdock-deploy/knowledge/${FILE_NAME}.md` |
| **H1** | `${H1_TITLE}` |
| **Pflicht-H2-Sub-Topics** (in dieser Reihenfolge) | ${H2_LIST} |
| **Anchor-Strings** (verbatim in den ersten Chunks der jeweiligen Sektionen) | ${ANCHORS} |
| **Marketing-Funktionen-Fokus** | ${MARKETING_FUNCTIONS} |
...
### S-${FILE_PREFIX}-001 [Szenario-Titel auf Deutsch]
...
Schreibe die Datei via `Write` direkt nach `little-data/langdock-deploy/knowledge/${FILE_NAME}.md`. Erstelle danach einen sauberen Commit auf deinem Working-Branch mit dem Format `little-data: author ${FILE_NAME} via Jules`.
...
> "Done. ${FILE_NAME}: [N_lines] lines, [N_bytes] bytes, [N_scenarios] scenarios. Method-distribution: [Mxx: count, ...]. Validation: PASS [optional notes]."
